    ...
    def run_eval_loop(sample_stochastically=True):
        ...
        for i in range(num_episodes):
            ...
            while not done:
                ...
                obs_frame = obs.reshape(4,3,84,84)
                obs_frame = obs_frame.transpose(0, 3, 2, 1)
                for i in range(len(obs_frame)):
                    single_frame = obs_frame[i]
                    frames_obs.append(single_frame)
                #video.record(env)
                episode_reward += reward

            video.save('%d.mp4' % step, frames_obs)
...
def main():
    ...
    #env = ObservationWrapper(env) #no obs wrapper for now
    env.seed(args.seed)
