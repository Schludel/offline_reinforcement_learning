    ...
    def run_eval_loop(sample_stochastically=True):
        ...
        for i in range(num_episodes): 
            ...
            while not done:
                ...
                terminal_dataset.append(done)
                timeouts_dataset.append(done) 
        ...
        dataset_filename = '%d_15.h5' % step
        dataset_dir = '/home/ws/ujvhi/rad/results/datasets/'
        path = os.path.join(dataset_dir, dataset_filename)
        expert_dataset.dump(path)
...
def main():
    ...
    print('obs_wrapper', env.observation_space)
    ...
    if args.encoder_type == 'pixel':
        ...
        print('frame_stack', env.observation_space)
        env = ActionRepeat(env, args.action_repeat)
        print('action_wrapper', env.observation_space)
    ...
    for step in range(args.num_train_steps):
        ...
        if done:
            if step > 0:
                ...
                start_time = time.time()
            if step % args.log_interval == 0:
                print('LOGGING EPSIODE REWARD TRAINING')
